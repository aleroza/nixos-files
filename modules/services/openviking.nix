...
#
# Single NixOS module, single systemd unit (docker-openviking.service
...
in
{
  ...
  config = lib.mkIf autoCfg.enable {
    ...
    # generates docker-openviking.service (or podman-openviking
    # depending on backend) that owns the container lifecycle.
    # Host-side hooks attached via systemd.services."docker-openviking"
    ...
    # through retries.
    systemd.services."docker-openviking" = {
      serviceConfig = {
        Restart = "on-failure";
        RestartSec = "30s";
        StartLimitBurst = 20;
